...
@section('content')
                ...
                <div class="col-md-5">
                    ...
                    <div class="hs-card">
                        ...
                        <textarea name="career_objective" class="form-control" rows="8"
                            placeholder="Enter your career objective...">{{ $about->career_objective }}</textarea>
                    </div>
                </div>
                            ...
                            <div class="d-flex gap-2">
                                <div class="dropdown">
                                    ...
                                    <ul class="dropdown-menu dropdown-menu-end shadow border-0">
                                        ...
                                        @endphp
                                        @foreach($quickFields as $label => $val)
                                            @if($val)
                                                <li><a class="dropdown-item" href="javascript:void(0)" onclick="addField('{{ $label }}', '{{ $val }}', '{{ str_contains(strtolower($label), 'birthday') ? 'date' : 'text' }}')">{{ $label }}</a></li>
                                            @endif
                                        @endforeach
                                    </ul>
                                </div>
                                    ...
                                    <div class="hs-detail-row mb-3">
                                        <div class="d-flex align-items-center gap-2 border rounded p-2 bg-light">
                                            ...
                                            <input type="text" name="labels[]" class="form-control form-control-sm w-25 hs-label-input"
                                                value="{{ $item['label'] }}" placeholder="Label" onfocus="showLabelPopup(this)">
    ...
    <script>
        ...
        const PROFILE_DATA = {
            'Name': @json($siteOwner->name),
            'Email': @json($siteOwner->email),
            'Phone': @json($siteOwner->phone ?? ''),
            'Address': @json($siteOwner->address ?? ''),
        };

        // Dynamically fetch and merge all items from "Personal Information Details"
        const personalDetails = @json($siteOwner->additional_info['personal'] ?? []);
