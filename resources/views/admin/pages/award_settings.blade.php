            ...
            <div class="hs-card mb-4">
                ...
                <div class="row">
                    <div class="col-md-12 mb-3">
                        ...
                        <input type="text" name="title" class="form-control" value="{{ $settings->title }}" placeholder="e.g. Awards & Certifications">
                    </div>
                    <div class="col-md-12">
                    ...
                    <div class="hs-card mb-4 award-item position-relative" data-index="{{ $index }}">
                        ...
                        <div class="row mb-3">
                            <div class="col-md-2">
                                ...
                                <input type="text" name="years[]" class="form-control" value="{{ $award->year }}" placeholder="e.g. 10 Sep 2025">
                            </div>
                            <div class="col-md-5">
                                <label class="form-label small fw-bold">Award / Certificate Title</label>
                                <input type="text" name="titles[]" class="form-control" value="{{ $award->title }}" placeholder="Certificate Name">
                            </div>
                            <div class="col-md-5">
                                <label class="form-label small fw-bold">Issuing Organization</label>
                                <input type="text" name="organizations[]" class="form-control" value="{{ $award->organization }}" placeholder="e.g. HackerRank">
                            </div>
                        </div>

                        <div class="mb-3">
                            <label class="form-label small fw-bold">Description (Optional)</label>
                            <textarea name="descriptions[]" class="form-control" rows="2" placeholder="Brief details about this achievement...">{{ $award->description }}</textarea>
                        </div>
                                    ...
                                    @foreach($award->proofs as $pIndex => $proof)
                                        <div class="proof-item mb-2">
                                            <div class="input-group input-group-sm">
                                                <input type="text" name="proofs[{{ $index }}][{{ $pIndex }}][label]" class="form-control" value="{{ $proof['label'] ?? 'Certificate' }}" style="max-width: 120px;">
                                                <input type="text" name="proofs[{{ $index }}][{{ $pIndex }}][value]" class="form-control proof-value-input" value="{{ $proof['value'] ?? '' }}" placeholder="Link or File URL">
                                                ...
                                                <div class="input-group-text bg-white">
                                                    <div class="form-check form-switch mb-0">
                                                        <input class="form-check-input proof-lock-toggle" type="checkbox" name="proofs[{{ $index }}][{{ $pIndex }}][is_protected]" value="1" {{ isset($proof['is_protected']) && $proof['is_protected'] ? 'checked' : '' }}>
                                                        <i class="ti ti-lock ms-1 {{ isset($proof['is_protected']) && $proof['is_protected'] ? 'text-danger' : 'text-muted' }}"></i>
                                                    </div>
                                                </div>
                                                
                                                <input type="text" name="proofs[{{ $index }}][{{ $pIndex }}][password]" class="form-control proof-password-input {{ isset($proof['is_protected']) && $proof['is_protected'] ? '' : 'd-none' }}" value="{{ $proof['password'] ?? '' }}" placeholder="Password" style="max-width: 100px;">
