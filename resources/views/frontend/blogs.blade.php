    ...
    <main class="main">
        <div class="container py-5">
                    ...
                    <div class="dropdown">
                        ...
                        <ul class="dropdown-menu w-100 shadow-sm border-orange" aria-labelledby="categoryFilter">
                            ...
                            @foreach($categories as $category)
                                <li class="dropdown-header fw-bold text-dark pt-2 pb-1" style="font-size: 0.9rem;">{{ $category->name }}</li>
                                <li><a class="dropdown-item py-1 category-filter-item ps-4" data-slug="{{ $category->slug }}" style="font-size: 0.85rem;" href="#">- Show All {{ $category->name }}</a></li>
                                @foreach($category->children as $child)
                                    <li><a class="dropdown-item py-1 category-filter-item ps-4" data-slug="{{ $child->slug }}" style="font-size: 0.85rem;" href="#">{{ $child->name }}</a></li>
                                @endforeach
                            @endforeach
                        </ul>
                    </div>
            ...
            <div id="blog-container" style="transition: opacity 0.3s ease;">
                @include('frontend.include.blog_list')
            </div>
        </div>
    </main>
